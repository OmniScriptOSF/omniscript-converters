...
export class XLSXConverter implements Converter {
  ...
  private async generateWorksheets(
  ...
  private addDocContentToWorksheet(
    worksheet: ExcelJS.Worksheet,
    doc: DocBlock,
    startRow: number
  ): number {
  ...
  private addSlideContentToWorksheet(
    worksheet: ExcelJS.Worksheet,
    slide: SlideBlock,
    startRow: number
  ): number {
  ...
  private applyWorksheetStyling(worksheet: ExcelJS.Worksheet, options: ConverterOptions): void {
    ...
    worksheet.columns = [{ width: 20 }, { width: 30 }, { width: 15 }, { width: 15 }];
